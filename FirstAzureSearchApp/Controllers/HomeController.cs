    ...
    public class HomeController : Controller
    {
        ...
        private async Task<ActionResult> RunQueryAsync(SearchData model)
        {
            ...
            if (model.searchText.Contains('+'))
            {
                ...
                // torrance + * + 
                filterQuery = "search.in(Office, '" + filterQuery + "', ',') or search.in(ClientID, '" + filterQuery + "', ',') or search.in(BuisnessUnit, '" + filterQuery + "', ',') or search.in(Region, '" + filterQuery + "', ',')";
            ...
            else
            {
                ...
                {
                    IncludeTotalCount = true,
                    Filter = "search.in(Office, '" + model.searchText + "', ',') or search.in(ClientID, '" + model.searchText + "', ',') or search.in(BuisnessUnit, '" + model.searchText + "', ',') or search.in(Region, '" + model.searchText + "', ',')",
                    OrderBy = { "" },                    
                    Size = 20
                };
            }
           ...
           Azure.Search.Documents.Models.SearchResults<Client> response = await _srchclient.SearchAsync<Client>(model.searchText, options);
            ...
            model.totalCount=response.TotalCount.Value.ToString();
            // Display the results.
